...
for var in PROJECT DUMPNAME INSTANCE; do
    if [[ -z "${!var:-}" ]];
    then
...
$SSH_CMD "neo4j-enterprise-causal-cluster-1-core-vm-$INSTANCE" --command "gsutil cp gs://$PROJECT/dumps/$DUMPNAME /tmp/bitcoin.dump"
echo "$(date -Iseconds) Downloading dump from bucket to neo4j-enterprise-causal-cluster-1-core-vm-$INSTANCE"
